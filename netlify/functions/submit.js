...
        });
      }
    } else if (data.committee === 'spectator') { // New logic for spectators
      // Save spectator data
      await collection.insertOne({
...
        email: data.email,
        phone: data.phone,
        committee: 'Spectator Pass', // Use a clear label
        spectatorPackage: data.spectatorPackage,
        registrationDate: new Date()
      });
    } else {
      // Regular single registration for all other committees
      await collection.insertOne({
        ...data,
        registrationDate: new Date()
      });
...
        registrationType: data.committee === 'ipl' ? 'team' : 'single'
      })
    };
    
  } catch (error) {
